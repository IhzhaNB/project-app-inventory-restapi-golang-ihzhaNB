...
func (sh *ShelfHandler) FindAll(w http.ResponseWriter, r *http.Request) {
	...
	// Call service
	shelves, pagination, err := sh.service.Shelf.FindAll(r.Context(), page, limit)
	if err != nil {
		sh.log.Error("Failed to get shelves", zap.Error(err))
	...
	utils.ResponseSuccess(w, http.StatusOK, "Shelves retrieved successfully", response)
}
