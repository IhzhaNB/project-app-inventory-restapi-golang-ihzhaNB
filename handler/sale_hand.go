...
// GetSalesReport handles GET /api/admin/sales/report - generates sales report
func (sh *SaleHandler) GetSalesReport(w http.ResponseWriter, r *http.Request) {
	...
	if startDate == "" || endDate == "" {
		utils.ResponseError(w, http.StatusBadRequest, "start_date and end_date are required", nil)
		return
	}

	// Prepare request DTO
	req := sale.SalesReportRequest{
		StartDate: startDate,
		EndDate:   endDate,
	}
	...
	report, err := sh.service.Sale.GetSalesReport(r.Context(), req)
	if err != nil {
		sh.log.Error("Failed to get sales report", zap.Error(err))
	...
	utils.ResponseSuccess(w, http.StatusOK, "Sales report generated successfully", report)
}
